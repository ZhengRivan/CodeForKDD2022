...
## Set up
##======================================================================
data = read.csv("real_16.csv", header = TRUE)
...
##----------------------------------------------------------------
Trans_data = data$Series[(P+1):nrow(data)] - data$Series[1:(nrow(data)-P)]
plot(Trans_data, type = "l")

arma_spec = autoarfima(Trans_data[1:(round(nrow(data)*Train_ratio))], 
                       criterion = "BIC", 
                       method = "full", 
                       include.mean = TRUE)
arma_spec
...
##======================================================================
##----------------------------------------------------------------
## Function of De-seasonal
##----------------------------------------------------------------
Diff_Op = function(P, M, data, i)
{
  
  data1 = data$Series[(i-M):i]
  data2 = data$Series[(i-P-M):(i-P)]
...
##----------------------------------------------------------------
## Function of Reverse De-seasonal and Forecast
##----------------------------------------------------------------
Rev_Op = function(P, M, data, i, Forecast_temp)
{
  data2 = data$Series[(i-P-M):(i-P)]
  
  corr_data = as.numeric(Forecast_temp@forecast$seriesFor) + data2[M + 1]
...
Z_stat = function(M, N, Train_ratio, data)
{
  ...
  for (i in (P+M+1):round(Train_ratio*N))
  {
    if(sum(data$anomaly_flag[(i-M):i]) == 0 && sum(data$anomaly_flag[(i-P-M):(i - P)]) == 0)
    {
      new_data = Diff_Op(P, M, data, i)
      ...
      arma_garch_model1 = ugarchfit(arma_garch_spec, data = new_data[1:(M - 1)], solver="hybrid")
      arma_garch_model2 = ugarchfit(arma_garch_spec, data = new_data, solver="hybrid")
...
anomaly_detect = function(est, Train_ratio, M, N, data)
{
  ...
  {
    new_data = Diff_Op(P, M, test_data, i)
    arma_garch_spec = ugarchspec(variance.model = list(model = "sGARCH",
                                                       garchOrder = c(1, 1),
                                                       submodel = NULL,
                                                       external.regressors = NULL,
    ...
    {
      anomaly_detect[i] = 1
      Raw_value[i] = test_data$Series[i]
      
      ##----------------------------------------------------------------
      ## Correct anomaly by Using Forecast Value
      ##----------------------------------------------------------------
      
      Forecast_temp = ugarchforecast(arma_garch_model1,
                                     n.ahead = 1,
                                     data = new_data[1:(M - 1)])
      test_data$Series[i] = Rev_Op(P, M, data, i, Forecast_temp) 
      
      New_value[i] = test_data$Series[i]
    }else{
      anomaly_detect[i] = 0
      Raw_value[i] = test_data$Series[i]
      New_value[i] = 0
...
Bagging = function(M_list, N, Train_ratio, data, Trim_num)
{
  ...
  for (i in 1:M_num)
  {
    Train_Result = Z_stat(M = 190, N, Train_ratio, data)
    ...
    ##----------------------------------------------------------------
    ## anomaly Detection based on Training Result
    ##----------------------------------------------------------------
    temp = anomaly_detect(est, Train_ratio, M =  M_list[i], N, data)
    Test_result = temp[(is.na(temp$Z_stat) !=TRUE), ]
    
    hist(Test_result$Z_stat, main = paste("Histogram of m = ", M_list[[i]], "in Test Set", sep = " "), breaks = 10)
